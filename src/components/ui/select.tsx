...
const Select = <T extends object>({
	...
	return (
		<SelectPrimitive
			{...props}
			className={ctr(className, "group flex w-full flex-col gap-y-1.5")}
			ref={selectRef}
		>
...
const Trigger = ({ className, ...props }: TriggerProps) => {
	return (
		...
		>
			...
			<SelectValue className="flex-1 [&_[slot=description]]:hidden placeholder-shown:text-muted-fg text-sm truncate" />
			<IconChevronLgDown
				aria-hidden
