...
const cellRangeStyles = tv({
  extend: focusRing,
  base: "flex h-full w-full items-center tabular-nums justify-center rounded-full data-unavailable:text-muted-fg/70 forced-color-adjust-none",
  variants: {
    selectionState: {
          ...
          {(date) => (
            <CalendarCell
              ...
              className={twJoin([
                ...
                "invalid:selected:bg-red-100 dark:invalid:selected:bg-red-700/30 data-[unavailable=true]:text-muted-fg/70"
              ])}
            >
              {({
                formattedDate,
